...
//  +-----------------------------------------------------------------------------+
__global__ void writeToRenderTargetKernel( const float4* accumulator, const int scrwidth, const int scrheight, cudaSurfaceObject_t RTSurface )
{
...
__host__ void writeToRenderTarget( const float4* accumulator, const int w, const int h, cudaSurfaceObject_t RTsurface )
{
	const dim3 gridDim( NEXTMULTIPLEOF( w, 32 ) / 32, NEXTMULTIPLEOF( h, 8 ) / 8 ), blockDim( 32, 8 );
	writeToRenderTargetKernel << < gridDim, blockDim >> > (accumulator, w, h, RTSurface);
}